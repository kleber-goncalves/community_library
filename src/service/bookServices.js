import bookRepository from "../repositories/bookRepositories.js";
...
async function findAllBooksService() {
    ...
    if (!books) throw new Error("Books não encontrado");
    return books;
}
...
async function updateBookService(updatedBook, bookId, userId) {
    const book = await bookRepository.findBookByIdRepository(bookId);
    if (!book) throw new Error("Book not found to update");

    // verifica se o user atual eh o dono do livro para poder atualizar
...
async function deleteBookService(bookId, userId) {
    ...
    // verifica se o user atual eh o dono do livro para poder deletar
    if (book.userId !== userId) throw new Error("Unauthorized");
